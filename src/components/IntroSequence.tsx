...
const CURTAIN_MS = 920; // the CSS entrance owns the screen until here
...
export default function IntroSequence() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      // Wait out whatever is left of the CSS entrance before the zoom.
      const d = Math.max(0, CURTAIN_MS - (performance.now() - t0));
      ...
      play(
        ...
        { duration: 560, delay: d, fill: "forwards", easing: "cubic-bezier(.5,0,.9,.6)" }
      );

      // The wordmark it leaves behind.
      play(
        flight,
        [
        ...
        { duration: 440, delay: d + 240, easing: "cubic-bezier(.22,.61,.36,1)" }
      );
    ...
    // Measuring the wordmark before Fraunces lands would size the flight to the
    // fallback serif. The monogram is an SVG, so the opening beat plays on time
    // either way.
